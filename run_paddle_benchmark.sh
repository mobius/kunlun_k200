...
echo "=== Installing paddlepaddle_xpu ==="
pip install https://paddle-whl.bj.bcebos.com/paddlex/xpu/paddlepaddle_xpu-2.6.1-cp310-cp310-linux_x86_64.whl

echo "=== Verifying Paddle XPU (single card) ==="
python -c "import paddle; print('Paddle version:', paddle.__version__); paddle.set_device('xpu:0'); print('XPU place OK')"

echo "=== Writing ResNet-50 benchmark script ==="
mkdir -p /mnt/storage/test_xpu/results

cat << 'PYEOF' > /mnt/storage/test_xpu/resnet50_benchmark.py
import paddle
...
python /mnt/storage/test_xpu/resnet50_benchmark.py
