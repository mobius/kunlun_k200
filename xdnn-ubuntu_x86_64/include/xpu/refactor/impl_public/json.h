#ifndef BAIDU_XPU_API_INCLUDE_XPU_REFACTOR_IMPL_JSON_H
#define BAIDU_XPU_API_INCLUDE_XPU_REFACTOR_IMPL_JSON_H
...
public:
    ...
    // Implicit constructor: vector_pair-like objects
    template <class M,
    ...
    // Return the enclosed value if this is a number, 0 otherwise. Note that json does not
    ...
    // Return the enclosed std::vector<std::pair>> if this is an object, or an empty vector<std::pair>> otherwise.
    ...
    parse_multi(const std::string& in, std::string& err, JsonParse strategy = JsonParse::STANDARD) {
        std::string::size_type parser_stop_pos;
