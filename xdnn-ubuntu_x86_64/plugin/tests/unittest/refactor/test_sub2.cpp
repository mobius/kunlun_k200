...
template<typename T> static void gtest_sub(api::DeviceType dev, const std::string& x_pos,
    ...
    ASSERT_EQ(0, plugin::sub2(&ctx_xpu, x1ptr, y1ptr, len));
    //plugin::sub2(&ctx_xpu, x1ptr, y1ptr, len);
    GTEST_XPU_END_DMA_FMT(&ctx_xpu, len * 2 * sizeof(T), "sub");

    GTEST_CPU_START(&ctx_cpu);
    //plugin::sub2(&ctx_cpu, x0ptr, y0ptr, len);
    ASSERT_EQ(0, plugin::sub2(&ctx_cpu, x0ptr, y0ptr, len));
    TENSOR_ALLCLOSE(y0, y1, maxdiff, maxdiff);
    GTEST_CPU_END(&ctx_cpu);
}

TEST(test_sub, function_xpu2) {
    gtest_sub<float>(api::kXPU2, GM, GM, 12345);
}
