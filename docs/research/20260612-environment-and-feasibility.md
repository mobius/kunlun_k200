...
> ⚠️ **开工前调研快照**。文中「当前为 DKMS 原版」「mmap 空 stub」等是 **2026-06-12 状态**。  
> 补救结果与现网数字见 `docs/impl/20260715-project-closure.md` / `README.md`。
...
| 项目 | 实测值（脱敏） | 评估 |
|------|--------|------|
| 型号 | AMD EPYC 类（服务器级 x86_64） | ✅ 满足内核编译与并行测试 |
| 架构 | x86_64 | ✅ |
| 在线核心 | 数十核级（SMT 开） | ✅ |
| 内核 | 6.8.x-generic | ✅ 与驱动 conftest 可匹配 |
| GCC | 12.x | ✅ 可编译 kunlun.ko |
| Make | 4.x | ✅ |
...
| 设备 | 用途 |
|------|------|
| AMD MI50 / Vega20 类（可选同机） | 性能对比基准，不参与 K200 驱动开发 |
| BMC VGA | 管理口，无关 |
...
| 项目 | 实测值（脱敏） | 评估 |
|------|--------|------|
| 卡数 | ≥2（PCI ID `1d22:3684`；BDF 本机 `lspci` 自查） | ✅ |
| XPU 设备 | `/dev/xpu*`、`/dev/xpuctrl` | ✅ 可访问 |
| 型号 | K200 (KL1) | ✅ |
| 固件 | 0001.0016.00xx 线 | ✅ |
...
| 工具 | 状态 | 用途规划 |
|------|------|----------|
| uv 0.11.11 | ✅ `uv (user-local install)` | Python 脚本/基准测试本地 venv（首选） |
