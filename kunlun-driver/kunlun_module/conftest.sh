    ...
    case "$1" in
       ...
       nvhost_dma_fence_unpack)
        ...
        iommu_get_domain_for_dev)
            #
            # Determine if iommu_get_domain_for_dev() function is present
            #
            # iommu_get_domain_for_dev() function was added by commit
            # 2c1296d92ac0367364bcb73a43c12a0bdfbfee75 (iommu: Add iommu_get_domain_for_dev function) in v4.1.
            # (2015-05-28).
            CODE="
            #include <linux/iommu.h>
            void conftest_iommu_get_domain_for_dev(void) {
                iommu_get_domain_for_dev();
            }"

            compile_check_conftest "$CODE" "NV_IOMMU_GET_DOMAIN_FOR_DEV_PRESENT" "" "functions"
        ;;

        iommu_map_has_gfp_arg)
            #
            # Determine if iommu_map() function has gfp parameter
            #
            # iommu_map() gfp paramter was added by commit 1369459(iommu: Add a gfp parameter to iommu_map())
            # in v6.3 (2023-01-25).
            CODE="
            #include <linux/iommu.h>
            #include <linux/types.h>
        ...
        class_dev_uevent_has_const_dev_args)
            #
            # Determine if class.dev_uevent() takes a const struct device *
            #
            # 23680f0b7d7f67a935adb38058110d2d81bbe6ea (2022-11-23).
            #
            ...
            # callback.
            #
            # 解决Linux 6.2+版本中一处参数类型增加const导致的编译失败
            #
            # /runtime/kunlun_module/kunlun/kl_main.c: In function 'kl_init':
            # /runtime/kunlun_module/kunlun/kl_main.c:544:25: error: assignment to 'int (*)(const struct device *, struct kobj_uevent_env *)' from incompatible pointer type 'int (*)(struct device *, struct kobj_uevent_env *)' [-Werror=incompatible-pointer-types]
            #   544 |     g_class->dev_uevent = kl_dev_uevent;
            #       |                         ^
            # cc1: some warnings being treated as errors
            #
            CODE="int main1(int argc, char **argv) { return 0; }"
            echo "$CODE" > conftest$$.c
            $CC $CFLAGS -Werror=incompatible-pointer-types -c conftest$$.c > /dev/null 2>&1
            rm -rf conftest$$.c
            if [ -f conftest$$.o ]; then
                # 编译器支持-Werror=incompatible-pointer-types选项
                rm -rf conftest$$.o

                CODE="$CONFTEST_PREAMBLE
                #include <linux/kobject.h>
                #include <linux/device.h>
                int kl_dev_uevent(struct device *dev, struct kobj_uevent_env *env) { return 0; }
                int func(void) {
                    struct class c;
                    c.dev_uevent = kl_dev_uevent;
                    return 0;
                }
                "
                echo "$CODE" > conftest$$.c
                ...
                rm -rf conftest$$.c
                if [ -f conftest$$.o ]; then
                    # 编译成功，函数指针第一个参数为struct device *
                    rm -rf conftest$$.o
                    echo "#undef NV_CLASS_DEV_UEVENT_HAS_CONST_DEV_ARGS" | append_conftest "functions"
                else
                    # 编译失败，函数指针第一个参数为const struct device *
                    echo "#define NV_CLASS_DEV_UEVENT_HAS_CONST_DEV_ARGS" | append_conftest "functions"
                fi
            else
                # 编译器不支持该选项，gcc-5.0.0 以上版本提供支持
                # 侧面也说明kernel版本较低，函数指针第一个参数为struct device *
                echo "#undef NV_CLASS_DEV_UEVENT_HAS_CONST_DEV_ARGS" | append_conftest "functions"
            fi
        ;;
        ...
        class_create_has_owner_arg)
            #
            # Determine if marco 'class_create' has owner param
            # commit 1aaba11da9aa7d
            #
            CODE="
            #include <linux/module.h>
            #include <linux/device.h>
            int conftest_class_create_has_owner(struct module *owner, const char *name) {
                class_create(owner, name);
            }"

            compile_check_conftest "$CODE" "NV_CLASS_CREATE_HAS_OWNER_ARG" "" "types"
        ;;
