...
enum class VariadicTableColumnAlign
{
    LEFT,
    RIGHT,
};
...
public:
  ...
  void setColumnAlign(const std::vector<VariadicTableColumnAlign> & column_align)
  {
    assert(column_align.size() == std::tuple_size<DataTuple>::value);
    _column_align = column_align;
  }
...
protected:
  ...
  void print_each(TupleType && t, StreamType & stream, std::integral_constant<size_t, I>)
  {
    ...
    if (_column_align.empty() || _column_align[I] == VariadicTableColumnAlign::LEFT)
    {
    ...
    {
        stream << std::string(_cell_padding, ' ') << std::setw(_column_sizes[I])
               << std::right << val << std::string(_cell_padding, ' ') << "|";
    }
  ...
  std::vector<VariadicTableColumnAlign> _column_align;
